...
HDA_INPUT_FILE           = r"C:\Users\M sD\Downloads\Data_rulesets_check\Tab_Files\HDA.tab"
INDEPENDENT_DEMAND_FILE  = r"C:\Users\M sD\Downloads\Data_rulesets_check\Tab_Files\IndependentDemand.tab"
...
class CustomerRuleEngine:
    ...
    def __init__(self, site_plants: set, hda_plants: set, ind_demand_plants: set):
        # Site table: PLANT column (used for SUPPLYINGPLANT validation)
        self.site_plants       = set(str(p).strip() for p in site_plants)

        # HDA.tab: PLANT column (used for CUSTOMER validation)
        self.hda_plants        = set(str(p).strip() for p in hda_plants)

        # IndependentDemand.tab: PRODUCTIONPLANT column (used for CUSTOMER validation)
        self.ind_demand_plants = set(str(p).strip() for p in ind_demand_plants)
    ...
    def validate_customer(self, row) -> bool:
        ...
        in_hda        = val_str in self.hda_plants
        in_ind_demand = val_str in self.ind_demand_plants
        return in_hda and in_ind_demand
    ...
    # Must NOT be blank AND must exist in PLANT column of Site table (Excel)
...
class CustomerTableValidator:
    """Loads Customer, Site, HDA, and IndependentDemand files, runs validation, builds error map."""

    def __init__(self, customer_path: str, site_path: str,
                 hda_path: str, ind_demand_path: str):
        ...
        self.hda_plants      = set()
        self.ind_demand_plants = set()
        self.error_map       = {}

    def load(self):
        # ── Customer table (Excel) ────────────────────────────────────────
        self.df = pd.read_excel(self.customer_path, dtype=str)
        self.df.columns = [c.strip().upper() for c in self.df.columns]

        # ── Site table (Excel) → PLANT column for SUPPLYINGPLANT rule ─────
        ...
        if "PLANT" not in site_df.columns:
            raise ValueError("PLANT column not found in Site table (Excel).")
...
class CustomerReportWriter:
    ...
    def _highlight(self, ws, df: pd.DataFrame, error_map: dict, col_index: dict):
        for df_idx, bad_cols in error_map.items():
            ...
            # Light yellow for the whole row
            for c in range(1, total_cols + 1):
                ws.cell(row=excel_row, column=c).fill = ROW_FILL
            # Red for each failing cell
...
class CustomerTableProcessor:
    ...
    def __init__(self, customer_path: str, site_path: str,
                 hda_path: str, ind_demand_path: str, output_path: str):
        self.validator = CustomerTableValidator(
            customer_path, site_path, hda_path, ind_demand_path
        )
